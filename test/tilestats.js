...
var xtend = require('../vendor/xtend');
...
test('TilestatsClient', function(tilestatsClient) {
  ...
  tilestatsClient.test('#putTilestats', function(putTilestats) {
    ...
    putTilestats.test('creates', function(assert) {
      var client = new MapboxClient(process.env.MapboxAccessToken);
      client.putTilestats(tilesetid, sampleStats, function(err, result) {
        assert.ifError(err, 'success');
        assert.deepEqual(result, xtend(sampleStats, {
          result: client.owner,
          tilesetid: tilesetid
        }), 'expected result');
        assert.end();
    ...
    putTilestats.test('updates', function(assert) {
      ...
      client.putTilestats(tilesetid, modifiedStats, function(err, result) {
        assert.ifError(err, 'success');
        assert.deepEqual(result, xtend(modifiedStats, {
          result: client.owner,
          tilesetid: tilesetid
        }), 'expected result');
        assert.end();
...
// LEGACY BELOW: Delete ASAP

test('TilestatsClient legacy', function(tilestatsClient) {
  var tilesetid = hat().slice(0, 6);
