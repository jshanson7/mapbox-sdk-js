...
MapboxDirections.prototype.getDirections = function(waypoints, options, callback) {
  ...
  if (options.geometries) {
    assert(allowedGeometries.indexOf(options.geometries) !== -1, 'geometries option must be ' + allowedGeometries);
    geometries = options.geometries;
  }

  var allowedOverviews = ['simplified', 'full'];
  if (options.overview) {
    assert(allowedOverviews.indexOf(options.overview) !== -1, 'overview option must be ' + allowedOverviews);
